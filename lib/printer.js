...
export class PrinterClient {
  ...
  _sendPacket = async (type, data, responseOffset = 1) => {
    debugLog('Writing packet!', type, data);
    this.transport.write(createPacketBytes(type, data));
  ...
  print = async (sharpImage, { density }) => {
    ...
    const { imageData, startRow } = await prepareImage(sharpImage);

    // META 1
    const meta1 = Buffer.alloc(3);
    meta1.writeInt8(0, 0);
    meta1.writeInt16BE(1, startRow);
    this.transport.write(createPacketBytes(RequestCode.IMAGE_DATA_META, meta1));
    // END META 1
...
export async function prepareImage(sharpImage) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    // Determine how many times this row can be repeated
    let repeat = 0;
    let nextColIndex = colIndex + width;
    let nextPixels = pixelArray.subarray(nextColIndex, nextColIndex + width);

    while (
      nextPixels.length > 0 &&
      nextPixels.every((pixel, index) => pixel === pixels[index])
    ) {
      repeat++;
      nextColIndex += width;
      nextPixels = pixelArray.subarray(nextColIndex, nextColIndex + width);
    }
    ...
    header.writeUInt16BE(repeat + 1, 4);
